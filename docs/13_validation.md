...
## باند‌های اندازه‌گیری

- **FIR (روش پنجره‌ای):** نقاط `fc1`/`fc2` در *میانهٔ* باند انتقال‌اند (هر کدام دقیقاً −6 dB). به همین دلیل باندِ اسکن‌شدهٔ «باند قطع» برای Lowpass/Highpass از لبهٔ درونی باند انتقال شروع می‌شود، و برای Bandstop روی **نصف درونی** باند قطع `[fc1+(fc2−fc1)/4, fc2−(fc2−fc1)/4]` اندازه‌گیری می‌شود — در غیر این صورت تضعیف گزارش‌شده ~6 dB می‌شد. (ripple باند عبور FIR تا نقطهٔ قطع، یعنی میانهٔ باند انتقال، سنجیده می‌شود؛ برای همین در طراحی‌های پنجره‌ای حدود 6 dB دیده می‌شود.)
- **IIR:** `fc1`/`fc2` لبه‌های مشخصه (−3dB یا لبهٔ ripple) هستند، بنابراین اسکن باند قطع روی تمام `[fc1, fc2]` است و در لبه‌ها دقیقاً مقدار spec را نشان می‌دهد.
- اگر بهرهٔ یک نقطه به کف عددی `1e−300` برسد (مثلاً صفر پاسخ در DC/Nyquist)، مقدار dB به −6000 کلاپ می‌شود و گزارش Markdown آن را به‌جای عدد خام `«< -600 dB»` نمایش می‌دهد.
- **Group delay** در نقاطی که پاسخ روی کف عددی باشد (صفرهای پاسخ، مثل DC یک Highpass) تعریف‌نشده است و `0.0` گزارش می‌شود.
