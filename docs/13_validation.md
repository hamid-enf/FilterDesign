...
- **FIR (روش پنجره‌ای):** نقاط `fc1`/`fc2` در *میانهٔ* باند انتقال‌اند (هر کدام دقیقاً −6 dB). بنابراین:
  - **ripple باند عبور** روی باندی اندازه‌گیری می‌شود که از هر لبهٔ باند عبور، به اندازهٔ **نصف عرض باند انتقال** (لبهٔ main-lobe پنجره: `fce_window_transition_half_hz`) کم شده باشد — یعنی فقط بخش تخت باند عبور. در غیر این صورت مقدار ~6 dB (خودِ باند انتقال) گزارش می‌شد. مقادیر نصف‌عرض (به‌صورت ضریب `fs/M`): rectangular ۱، Bartlett/Hann/Hamming ۲، Blackman ۳، Blackman-Harris/Tukey(α=0.5) ۴؛ برای Kaiser نصف عرض انتقالِ طراحی `(A−7.95)/(2·2.285·(M−1))`. اگر باند عبور باریک‌تر از باند انتقال باشد (spec تخریب‌شده)، کل باند اندازه‌گیری می‌شود و ripple حداکثر ~6 dB می‌شود.
  - **تضعیف باند قطع** برای Bandstop روی **نصف درونی** باند قطع `[fc1+(fc2−fc1)/4, fc2−(fc2−fc1)/4]` اندازه‌گیری می‌شود — در غیر این صورت ~6 dB گزارش می‌شد. برای Lowpass/Highpass باند اسکن از میانهٔ فاصلهٔ cutoff تا لبهٔ باند شروع می‌شود (نگرانی).
  - ripple باند عبور Bandstop **بدترین** مقدار دو باند عبور پایین و بالاست (هم‌ساز با IIR).
